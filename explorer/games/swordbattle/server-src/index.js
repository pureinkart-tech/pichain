...
function setCorsHeaders(response) {
  ...
  response.writeHeader('Access-Control-Allow-Methods', 'GET, POST');
  response.writeHeader('Access-Control-Allow-Headers', 'content-type');
}

// CORS preflight for /add-bot
app.options('/add-bot', (res) => {
  setCorsHeaders(res);
  res.writeStatus('204 No Content');
  res.end();
});

// REST: POST /add-bot — spawn AI bots using the external swordbattle-bot.js
app.post('/add-bot', (res, req) => {
  let body = '';
  res.onAborted(() => {});
  res.onData((chunk, isLast) => {
    ...
    if (isLast) {
      try {
        ...
        const botScript = path.resolve(__dirname, '../../../../test-bots/swordbattle-bot.js');
        ...
        bot.on('close', code => console.log(`[BOT] Swordbattle bot process exited with code ${code}`));
        console.log(`[Swordbattle] Spawned ${numBots} AI bot(s) for room ${roomId} (pid: ${bot.pid})`);
        setCorsHeaders(res);
