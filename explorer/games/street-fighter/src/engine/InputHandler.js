import { CONTROLLER_DEADZONE, controls, JUMP_ALIAS } from '../config/controls.js';
...
let pvpMode = false;
let localPlayerId = 0;
const remoteHeldControls = new Set();
const remoteConsumedPresses = new Set();
const remoteConsumedPressesHistory = new Set();

export const setPvPMode = (enabled, myPlayerId = 0) => {
	pvpMode = enabled;
	localPlayerId = myPlayerId;
	remoteHeldControls.clear();
	remoteConsumedPresses.clear();
	remoteConsumedPressesHistory.clear();
};

export const updateRemoteInputs = (heldControlIds) => {
	// Detect new presses: controls freshly added get their consumed state cleared
	const newSet = new Set(heldControlIds);
	for (const c of newSet) {
		if (!remoteHeldControls.has(c)) {
			remoteConsumedPresses.delete(c);
			remoteConsumedPressesHistory.delete(c);
		}
	}
	// Controls released: clear from consumed sets
	for (const c of remoteHeldControls) {
		if (!newSet.has(c)) {
			remoteConsumedPresses.delete(c);
			remoteConsumedPressesHistory.delete(c);
		}
	...
	remoteHeldControls.clear();
	newSet.forEach(c => remoteHeldControls.add(c));
};

export const getLocalHeldControls = () => {
	const held = [];
	const kb = controls[localPlayerId].keyboard;
	for (const controlId of Object.keys(kb)) {
		if (heldKeys.has(kb[controlId])) held.push(controlId);
	}
	// Check gamepad too
	const gp = controls[localPlayerId].gamepad;
	for (const controlId of Object.keys(gp)) {
		if (!held.includes(controlId) && heldGamepadButtons[localPlayerId].has(gp[controlId])) {
			held.push(controlId);
		}
	}
	return held;
...
const isRemoteHeld = (controlId) => remoteHeldControls.has(controlId);

const isRemotePressed = (controlId) => {
	if (remoteHeldControls.has(controlId) && !remoteConsumedPresses.has(controlId)) {
		remoteConsumedPresses.add(controlId);
		return true;
	}
	return false;
};

const isRemotePressedHistory = (controlId) => {
	if (remoteHeldControls.has(controlId) && !remoteConsumedPressesHistory.has(controlId)) {
		remoteConsumedPressesHistory.add(controlId);
		return true;
	}
	return false;
};
...
const handleKeyDown = (event) => {
	const code = resolveKey(event.code);
...
// ============================================
// Directional Input Queries (PvP-aware)
// ============================================
export const isLeft = (id) => {
	if (pvpMode && id !== localPlayerId) return isRemoteHeld(Control.LEFT);
...
export const isUp = (id) => {
	if (pvpMode && id !== localPlayerId) return isRemoteHeld(Control.UP);
...
export const isRight = (id) => {
	if (pvpMode && id !== localPlayerId) return isRemoteHeld(Control.RIGHT);
...
export const isDown = (id) => {
	if (pvpMode && id !== localPlayerId) return isRemoteHeld(Control.DOWN);
...
// ============================================
// Attack Input Queries (PvP-aware)
// ============================================
export const isKeyPressed = (id, code, forControlHistory) => {
	if (pvpMode && id !== localPlayerId) {
		return forControlHistory ? isRemotePressedHistory(code) : isRemotePressed(code);
	}
